  ...
  module Instrumentation
    module Sinatra::Base
      ...
      def call_on_exit(event, retval)
        ...
        if NewRelic::Security::Agent.config[:'security.scan_controllers.report_http_response_body']
          http_response = NewRelic::Security::Agent::Control::HTTPResponse.new(*retval) if retval
          NewRelic::Security::Agent::Control::ReflectedXSS.check_xss(ctxt, http_response) if NewRelic::Security::Agent.config[:'security.detection.rxss.enabled']
          NewRelic::Security::Agent.agent.event_processor.send_http_response_event(NewRelic::Security::Agent::Control::HTTPResponseEvent.new(ctxt, http_response)) if ctxt.headers[NR_CSEC_FUZZ_REQUEST_ID]&.include?(VULNERABLE)
        end
